...
import { getNow, getMsInDay, getToday, getYear } from "@/api/getDates.js";
...
const actions = {
  ...
  async getSolution({ getters, commit }, nutrients) {
    ...
    commit("setDays");
    let { A, b, c, indices } = await getters.getConditions(nutrients);
    //min
    /*A = [
      [-1, -2, -3],
      [-5, -6, -7],
  ...
  _setCalcium({ state, dispatch }) {
    ...
    const age = (getNow() - birthdate) / getMsInDay() / 365.25;
